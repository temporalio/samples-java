...
temporal operator nexus endpoint create \
  ...
  --description-file ./core/src/main/java/io/temporal/samples/nexusmessaging/service/description.md
```
...
```
./gradlew -q execute -PmainClass=io.temporal.samples.nexusmessaging.handler.HandlerWorker \
...
```
./gradlew -q execute -PmainClass=io.temporal.samples.nexusmessaging.caller.CallerWorker \
...
```
./gradlew -q execute -PmainClass=io.temporal.samples.nexusmessaging.caller.CallerStarter \
    --args="-target-host localhost:7233 -namespace my-caller-namespace"
```
...
```
[main] INFO  i.t.s.nexusmessaging.caller.CallerStarter - Workflow result: Nexus Echo 👋 
```
