 ...
 *  process individual events in the
 */
public class HelloAccumulator {
  ...
  public interface AccumulatorWorkflow {
    ...
    @SignalMethod
    void waitforGreeting(Greeting greeting);
  ...
  public static class AccumulatorWorkflowImpl implements AccumulatorWorkflow {
    ...
    ArrayDeque<Greeting> unprocessedGreetings;
    ...
    public String accumulateGreetings(
      ...
      // If you want to wait for a fixed amount of time instead of a time after a message
      ...
      do {
        ...
        Workflow.await(MAX_AWAIT_TIME, () -> !unprocessedGreetings.isEmpty() || stopWaiting);
    ...
    @Override
    public void waitforGreeting(Greeting greeting) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    workflowSync.waitforGreeting(starterGreeting);

    // Test sending an exit, waiting for workflow exit, then sending a signal.
    // This will trigger a WorkflowNotFoundException if using the same workflow handle
    ...
    try {
      ...
      workflowById.waitforGreeting(laterGreeting);
       ...
       * blocks until a result is available. Note that this behavior assumes that WorkflowOptions
       * are not configured with WorkflowIdReusePolicy.AllowDuplicate. If they were, this call would fail
    ...
    /*
     * Here we try to send the signals as start to demonstrate that after a workflow exited
    ...
    if (testSignalAfterWorkflowExit) {
      ...
      BatchRequest request = client.newSignalWithStartRequest();
      request.add(workflow::accumulateGreetings, bucket, greetingList, allGreetingsSet);
      request.add(workflow::waitforGreeting, laterGreeting);
      client.signalWithStart(request);
